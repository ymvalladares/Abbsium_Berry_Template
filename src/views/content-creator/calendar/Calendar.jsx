  ...
  return (
    <Box sx={{ p: { xs: 1.5, sm: 2, md: 3 } }}>
      ...
      <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '14px', overflow: 'hidden' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2.5, py: 1.5, borderBottom: '1px solid', borderColor: 'divider', bgcolor: isDark ? '#1e293b' : '#fff' }}>
          <Typography sx={{ fontWeight: 800, fontSize: '1rem', color: isDark ? '#f1f5f9' : undefined }}>{MONTHS[month]} {year}</Typography>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <IconButton size="small" onClick={goBack}><ChevronLeft /></IconButton>
            <Button size="small" onClick={goToday} variant="outlined" sx={{ textTransform: 'none', fontWeight: 700, borderRadius: '8px', fontSize: '0.75rem', minWidth: 60, borderColor: 'divider', color: 'text.primary', '&:hover': { borderColor: '#5E35B1', color: isDark ? '#b388ff' : '#5E35B1' } }}>Today</Button>
            ...
            {Array.from({ length: 35 }).map((_, idx) => {
              ...
              return (
                  <Box key={idx} sx={{ borderRight, borderBottom, borderColor: 'divider', p: '6px 6px 4px', minHeight: 110 }}>
                    <Box sx={{ width: 26, height: 26, borderRadius: '50%', bgcolor: isDark ? '#334155' : '#f0f0f0', ml: 'auto', mb: '8px' }} />
                    <Box sx={{ width: '80%', height: 12, bgcolor: isDark ? '#334155' : '#f0f0f0', borderRadius: 2, mb: 0.5 }} />
                    <Box sx={{ width: '60%', height: 12, bgcolor: isDark ? '#334155' : '#f0f0f0', borderRadius: 2 }} />
                  </Box>
              );
            })}
          </Box>
      ...
      <Dialog open={!!detailEvent} onClose={() => setDetailEvent(null)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '16px', bgcolor: isDark ? '#1e293b' : undefined } }}>
        <Box sx={{ p: 3 }}>
          ...
          {detailEvent?.contentType && (
            <Chip size="small" label={detailEvent.contentType === 'reel' ? 'Reel/Short' : detailEvent.contentType}
              ...
              sx={{
                ...
                ...(isDark
                  ? {
                      bgcolor: detailEvent.contentType === 'reel' ? 'rgba(228,64,95,0.15)' : 'rgba(94,53,177,0.15)',
