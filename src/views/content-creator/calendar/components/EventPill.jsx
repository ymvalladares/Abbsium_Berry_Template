...
export default function EventPill({ event, onClick, compact }) {
  ...
  const bgColor = isScheduled ? alpha('#5E35B1', isDark ? 0.15 : 0.06) : alpha(event.platformColor || '#4CAF50', isDark ? 0.15 : 0.06);
    ...
    <Tooltip title={`${event.title}${event.time ? ' · ' + formatTime(event.time) : ''}`} arrow>
      <Box
        ...
        sx={{
          ...
          cursor: 'pointer',
          overflow: 'hidden',
          '&:hover': { filter: isDark ? 'brightness(1.3)' : 'brightness(0.94)' }
        }}
      >
