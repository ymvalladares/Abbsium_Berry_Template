      ...
      >
        ...
        >
          {isPlaying && videoLoaded && clip.videoUrl ? (
            ...
            >
              <Typography sx={{ fontSize: '0.7rem', color: '#ef4444', textAlign: 'center' }}>Video format not compatible</Typography>
              <Button
                size="small"
                variant="outlined"
                sx={{ color: '#fff', fontSize: '0.7rem', borderColor: alpha('#fff', 0.3), borderRadius: '12px' }}
              ...
              {clip.thumbnailUrl ? (
                ...
                <Box
                  sx={{
                    position: 'absolute',
                    inset: 0,
                    background: `linear-gradient(135deg, hsl(${clip.viralScore * 3.6}, 60%, 18%), hsl(${clip.viralScore * 3.6 + 40}, 50%, 8%))`,
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center'
                  }}
            ...
            <Stack spacing={1.5} alignItems="center" sx={{ py: 8 }}>
              <Box sx={{ position: 'relative', width: 52, height: 52 }}>
                <svg width="52" height="52" viewBox="0 0 52 52" style={{ transform: 'rotate(-90deg)' }}>
                  <circle cx="26" cy="26" r="22" fill="none" stroke={alpha('#2563eb', 0.1)} strokeWidth="3" />
                  <circle
                    cx="26"
                    cy="26"
                    r="22"
                    ...
                    strokeWidth="3"
                    strokeLinecap="round"
                    strokeDasharray={`${jobProgress} ${100 - jobProgress}`}
                    strokeDashoffset="0"
                    pathLength="100"
                ...
                <Box sx={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Typography sx={{ fontSize: '0.75rem', fontWeight: 800, color: '#2563eb' }}>{Math.round(jobProgress)}%</Typography>
                </Box>
              </Box>
              {!hasRealClips && (
        ...
        <Box sx={{ p: 2 }}>
          <Stack direction="row" alignItems="flex-start" justifyContent="space-between" sx={{ mb: 1.5 }}>
            <Box sx={{ flex: 1, mr: 2 }}>
              ...
              {clip.duration && <Typography sx={{ color: 'text.secondary', fontSize: '0.7rem' }}>{clip.duration}s</Typography>}
            </Box>
          ...
          {ready && (
            <>
              <Box sx={{ height: 1, background: `linear-gradient(90deg, transparent, ${alpha('#2563eb', 0.15)}, transparent)`, mb: 1.5 }} />

              <Box onClick={() => setShowDetails(!showDetails)} sx={{ cursor: 'pointer', py: 0.5 }}>
                <Stack direction="row" alignItems="center" justifyContent="space-between">
                  <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: '#2563eb', letterSpacing: '0.02em' }}>
                    AI Insights
                  </Typography>
                  {showDetails ? <IconChevronUp size={16} color="#2563eb" /> : <IconChevronDown size={16} color="#2563eb" />}
                </Stack>
              </Box>
              ...
              <Collapse in={showDetails}>
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5, mt: 1.5 }}>
                  {/* Viral Potential */}
                    ...
                    >
                      <IconTarget size={16} color={scoreColor} />
                    </Box>
                    <Typography
                      sx={{
                        fontSize: '0.62rem',
                    ...
                    <Typography sx={{ fontSize: '0.85rem', fontWeight: 800, color: 'text.primary', mt: 0.25, lineHeight: 1.3 }}>
                      {clip.viralScore >= 85
                        ? 'High engagement expected'
                        : clip.viralScore >= 70
                          ? 'Good potential for reach'
                          : 'Consider stronger hook'}
                    </Typography>
                    <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: scoreColor, mt: 0.75 }}>
                  ...
                  {/* Best Hook */}
                    ...
                    <Typography sx={{ fontSize: '0.85rem', fontWeight: 800, color: 'text.primary', mt: 0.25, lineHeight: 1.3 }}>
                      {clip.hooks?.[0] || 'Start with a question'}
                    </Typography>
                  </Box>
                  ...
                  {/* Best Time to Post */}
                    ...
                    >
                      <IconClock size={16} color="#10b981" />
                    </Box>
                    <Typography
                      sx={{
                        fontSize: '0.62rem',
            ...
            <Stack direction="row" spacing={2} alignItems="center">
              <Box
                sx={{
                  ...
                  height: 40,
                  borderRadius: '16px',
                  bgcolor: allReady ? alpha('#10b981', 0.12) : alpha('#2563eb', 0.08),
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center'
                }}
              >
                {allReady ? (
                  ...
                  <IconScissors size={20} color="#2563eb" />
                )}
              </Box>
              <Box sx={{ flex: 1 }}>
              ...
              <Box sx={{ mt: 2 }}>
                <LinearProgress
                  ...
                  sx={{
                    ...
                    '& .MuiLinearProgress-bar': { borderRadius: '12px', background: 'linear-gradient(90deg, #2563eb, #3b82f6)' }
                  }}
                />
              </Box>
            )}
            ...
            >
              ...
              <Button
                ...
                sx={{
                  ...
                  boxShadow: `0 4px 16px ${alpha('#2563eb', 0.3)}`,
                  '&:hover': { background: 'linear-gradient(135deg, #4527A0, #651FFF)', boxShadow: `0 6px 24px ${alpha('#2563eb', 0.4)}` }
                }}
              >
                Publish
              </Button>
              {onCleanup && (
                <Button
                  size="small"
                  startIcon={isCleaning ? <CircularProgress size={18} sx={{ color: '#ef4444' }} /> : <IconTrash size={18} />}
                  disabled={isCleaning}
                  onClick={async () => {
                    setIsCleaning(true);
                    await onCleanup();
